...
const seedTrips = async () => {
    try {
        ...
        const { data: cars } = await supabase.from('car').select('car_id, user_id');

        const cities = ['Paris', 'Lyon', 'Marseille', 'Nantes', 'Bordeaux'];
        const trips = Array.from({ length: 20 }).map(() => {
            const selectedCar = faker.helpers.arrayElement(cars);
            ...
            return {
                ...
                price_per_person: faker.number.int({ min: 15, max: 55 }),
                car_id: selectedCar.car_id,
                driver_id: selectedCar.user_id
            };
        });

        const { data, error } = await supabase.from('trip').insert(trips).select();
        if (error) throw error;
        console.log(`🚀 ${data.length} trips created!`);
    } catch (err) {
